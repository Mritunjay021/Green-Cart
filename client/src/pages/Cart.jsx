...
const Cart = () => {
    ...
    const {product ,axios , currency, getCartCount, getCartAmount, cartItems, updateCart, removefromCart, navigate, User , setCartItems} = useAppContext();
    ...
    useEffect(() => {
    const baseAmount = getCartAmount() + (getCartAmount() * 2 / 100); 
    if (paymentMethod === "COD") {
        setTotalAmount(Math.floor(baseAmount));
    } else {
        setTotalAmount(baseAmount);
    }
    }, [paymentMethod, cartItems, product]); 
    ...
    return product.length > 0 && cartItems? (
        <div className="flex flex-col md:flex-row mt-16">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <p className="text-gray-500">{selectedAddress ? `${selectedAddress.street}, ${selectedAddress.city}, ${selectedAddress.state}, ${selectedAddress.country}` : "No address found"}</p>
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                ...
                                map((address, index) => (
                                    <p key={index} onClick={() => {setShowAddress(false);setSelectedAddress(address)}} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-dull-primary transition">
